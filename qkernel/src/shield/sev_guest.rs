...
impl SnpGuestDev {
    ...
    pub fn init(&mut self, _vmpck_id : u32) -> () {
	 ...
	 fn prepare_emulated_guest_attestation_report (&self, user_data: &[u8] ) -> AttestationReport {

	
		let mut report: AttestationReport = AttestationReport::default();
	...
	pub fn get_report(&mut self, report_data: String) -> Result<String> {
		...
		let report = self.prepare_emulated_guest_attestation_report(&report_data_bin[..]);
