...
use crate::aes_gcm::aead::{OsRng, generic_array::{GenericArray, typenum::U32}, rand_core::RngCore};
    ...
    pub fn encrypNormalIOStdouterr (&self, src: DataBuff, inode_id: u64) -> Result<DataBuff> {
        ...
        match stdio_type {
            ...
            StdioType::SessionAllocationStdio(ref s) => {
                ...
                let mut encrypted_session_shared = Vec::new_in(GUEST_HOST_SHARED_ALLOCATOR);
                encrypted_session_shared.resize(encrypted_session.len(), 0);
                encrypted_session_shared.copy_from_slice(&encrypted_session[..]);
                buf.buf = encrypted_session_shared;
                return Ok(buf);
            },
            StdioType::PolicyUpdate(ref update_res) => {
                ...
                let mut encrypted_encoded_outBoundDate_in_shared = Vec::new_in(GUEST_HOST_SHARED_ALLOCATOR);
                encrypted_encoded_outBoundDate_in_shared.resize(encodedOutBoundDate.len(), 0);
                encrypted_encoded_outBoundDate_in_shared.copy_from_slice(&encodedOutBoundDate[..]);
                buf.buf = encrypted_encoded_outBoundDate_in_shared;       
                return Ok(buf);
            }
        }
        ...
        encrypted_encoded_outBoundDate_in_shared.resize(encodedOutBoundDate.len(), 0);
        encrypted_encoded_outBoundDate_in_shared.copy_from_slice(&encodedOutBoundDate[..]);
        res.buf = encrypted_encoded_outBoundDate_in_shared;  
...
fn verify_hmac (key_slice : &[u8], message: &String, base64_encoded_code: &String) -> bool {
 ...
 * ************************ ************************************************
 */
fn verify_privileged_exec_cmd(privileged_cmd: &mut Vec<String>, key_slice: &[u8], key: &GenericArray<u8, U32>) -> Result<Vec<String>>  {
