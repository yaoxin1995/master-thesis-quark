...
impl TermianlIoShiled for TerminalShield {
    ...
    fn termianlIoEncryption(&self, src: &[IoVec], task: &Task,  inode_id: u64) -> Result<(usize, Option<Vec::<IoVec>>)>{
        ...
        if tty_args.stdio_type == StdioType::ContaienrStdio && self.policy.privileged_user_config.enable_container_logs_encryption {
            ...
            let mut encodedOutBoundDateShared = Vec::new_in(GUEST_HOST_SHARED_ALLOCATOR);
            encodedOutBoundDateShared.resize(encodedOutBoundDate.len(), 0);
            encodedOutBoundDateShared.copy_from_slice(&encodedOutBoundDate[..]);

            encrypted_iov.buf = encodedOutBoundDateShared;
    ...
    fn write_buf(&self, task: &Task, to: i32, buf: &[u8]) -> Result<i64> {
        ...
        while offset < len {
            ...
            let mut bufShared = Vec::new_in(GUEST_HOST_SHARED_ALLOCATOR);
            bufShared.resize(count, 0);
            bufShared.copy_from_slice(but_to_write);

            src_buf.buf = bufShared;
            let len = src_buf.Len();
